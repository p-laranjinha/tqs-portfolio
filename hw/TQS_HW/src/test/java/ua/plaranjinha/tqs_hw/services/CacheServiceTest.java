...
class CacheServiceTest {
    ...
    @Mock(lenient = true)
    private WeatherBitService wbService;
    ...
    @Test
    void getDataFromLocationOWM() {
        Mockito.when(owmService
                .getDataFromLocation(currentData.getLocation(), currentData.getCountry_code()))
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocation(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(1, 0, 0, 0);
    }

    @Test
    void getDataFromLocationWB() {
        Mockito.when(owmService
                .getDataFromLocation(Mockito.anyString(), Mockito.anyString()))
                .thenReturn(null);
        Mockito.when(wbService
                .getDataFromLocation(currentData.getLocation(), currentData.getCountry_code()))
                .thenReturn(currentData);
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocation(Mockito.anyString(), Mockito.anyString());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromLocation(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(0, 1, 0, 0);
    }

    @Test
    void getDataFromLocationCache() {
        Mockito.when(owmService
                .getDataFromLocation(currentData.getLocation(), currentData.getCountry_code()))
                .thenReturn(currentData);
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocation(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(1, 0, 1, 0);
    }

    @Test
    void getDataFromCoordsOWM() {
        Mockito.when(owmService
                .getDataFromCoords(currentData.getCoordinates().getLat(), currentData.getCoordinates().getLon()))
                .thenReturn(currentData);
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromCoords(Mockito.anyDouble(), Mockito.anyDouble());
        checkServiceRequestNums(1, 0, 0, 0);
    }

    @Test
    void getDataFromCoordsWB() {
        Mockito.when(owmService
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromCoords(Mockito.anyDouble(), Mockito.anyDouble());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromCoords(Mockito.anyDouble(), Mockito.anyDouble());
        checkServiceRequestNums(0, 1, 0, 0);
    }

    @Test
    void getDataFromCoordsCache() {
        Mockito.when(owmService
                .getDataFromCoords(currentData.getCoordinates().getLat(), currentData.getCoordinates().getLon()))
                .thenReturn(currentData);
    ...
    @Test
    void getDataFromLocationHistoryOWM() {
        Mockito.when(owmService
                .getDataFromLocationHistory(historyData.getLocation(), historyData.getCountry_code()))
                .thenReturn(historyData);

        assertThat(
                cacheService.getDataFromLocationHistory(historyData.getLocation(), historyData.getCountry_code())).
                isEqualTo(historyData);

        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocationHistory(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(1, 0, 0, 0);
    }

    @Test
    void getDataFromLocationHistoryWB() {
        Mockito.when(owmService
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocationHistory(Mockito.anyString(), Mockito.anyString());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromLocationHistory(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(0, 1, 0, 0);
    }

    @Test
    void getDataFromLocationHistoryCache() {
        Mockito.when(owmService
                .getDataFromLocationHistory(historyData.getLocation(), historyData.getCountry_code()))
                .thenReturn(historyData);
    ...
    @Test
    void getDataFromCoordsHistoryOWM() {
        Mockito.when(owmService
                .getDataFromCoordsHistory(historyData.getCoordinates().getLat(), historyData.getCoordinates().getLon()))
                .thenReturn(historyData);
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromCoordsHistory(Mockito.anyDouble(), Mockito.anyDouble());
        checkServiceRequestNums(1, 0, 0, 0);
    }

    @Test
    void getDataFromCoordsHistoryWB() {
        Mockito.when(owmService
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromCoordsHistory(Mockito.anyDouble(), Mockito.anyDouble());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromCoordsHistory(Mockito.anyDouble(), Mockito.anyDouble());
        checkServiceRequestNums(0, 1, 0, 0);
    }

    @Test
    void getDataFromCoordsHistoryCache() {
        Mockito.when(owmService
                .getDataFromCoordsHistory(historyData.getCoordinates().getLat(), historyData.getCoordinates().getLon()))
                .thenReturn(historyData);
    ...
    @Test
    void getDataFromLocationForecastOWM() {
        Mockito.when(owmService
                .getDataFromLocationForecast(forecastData.getLocation(), forecastData.getCountry_code()))
                .thenReturn(forecastData);

        assertThat(
                cacheService.getDataFromLocationForecast(forecastData.getLocation(), forecastData.getCountry_code())).
                isEqualTo(forecastData);

        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocationForecast(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(1, 0, 0, 0);
    }

    @Test
    void getDataFromLocationForecastWB() {
        Mockito.when(owmService
        ...
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocationForecast(Mockito.anyString(), Mockito.anyString());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromLocationForecast(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(0, 1, 0, 0);
    }

    @Test
    void getDataFromLocationForecastCache() {
        Mockito.when(owmService
                .getDataFromLocationForecast(forecastData.getLocation(), forecastData.getCountry_code()))
                .thenReturn(forecastData);
    ...
    @Test
    void getDataFromCoordsForecastOWM() {
        Mockito.when(owmService
                .getDataFromCoordsForecast(forecastData.getCoordinates().getLat(), forecastData.getCoordinates().getLon()))
                .thenReturn(forecastData);
    ...
    @Test
    void getDataFromCoordsForecastWB() {
        Mockito.when(owmService
    ...
    @Test
    void getDataFromCoordsForecastCache() {
        Mockito.when(owmService
                .getDataFromCoordsForecast(forecastData.getCoordinates().getLat(), forecastData.getCoordinates().getLon()))
                .thenReturn(forecastData);
    ...
    void getDataFromCache() {
        ...
        FullCacheData emptyCacheData = new FullCacheData();
        emptyCacheData.setOwm_requests(0);
        emptyCacheData.setWb_requests(0);
        emptyCacheData.setCache_requests(0);
        emptyCacheData.setFailed_requests(0);
        emptyCacheData.setCurrent(new CacheData[0]);
        emptyCacheData.setHistory(new CacheData[0]);
        emptyCacheData.setForecast(new CacheData[0]);

        assertThat(cacheService.getDataFromCache()).isEqualTo(emptyCacheData);
        assertThat(
                cacheService.getDataFromCoordsForecast(forecastData.getCoordinates().getLat(), forecastData.getCoordinates().getLon())).
                isEqualTo(forecastData);
        assertThat(cacheService.getDataFromCache()).isNotEqualTo(emptyCacheData);
        assertThat(cacheService.getDataFromCache().getOwm_requests()).isEqualTo(1);
    }

    @Test
    void getDataFromLocationAllNull() {
        Mockito.when(owmService
                .getDataFromLocation(Mockito.anyString(), Mockito.anyString()))
                .thenReturn(null);
        Mockito.when(wbService
        ...
        assertThat(cacheService.getDataFromLocation("", "")).isEqualTo(null);
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocation(Mockito.anyString(), Mockito.anyString());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromLocation(Mockito.anyString(), Mockito.anyString());
    ...
    void getDataFromCoordsAllNull() {
        ...
        assertThat(cacheService.getDataFromCoords(0, 0)).isEqualTo(null);
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromCoords(Mockito.anyDouble(), Mockito.anyDouble());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromCoords(Mockito.anyDouble(), Mockito.anyDouble());
        checkServiceRequestNums(0, 0, 0, 1);
    }
    ...
    @Test
    void getDataFromLocationHistoryAllNull() {
        Mockito.when(owmService
                .getDataFromLocationHistory(Mockito.anyString(), Mockito.anyString()))
                .thenReturn(null);
        Mockito.when(wbService
        ...
        assertThat(cacheService.getDataFromLocationHistory("", "")).isEqualTo(null);
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocationHistory(Mockito.anyString(), Mockito.anyString());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromLocationHistory(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(0, 0, 0, 1);
    }
    ...
    @Test
    void getDataFromCoordsHistoryAllNull() {
        Mockito.when(owmService
                .getDataFromCoordsHistory(Mockito.anyDouble(), Mockito.anyDouble()))
                .thenReturn(null);
        Mockito.when(wbService
        ...
        assertThat(cacheService.getDataFromCoordsHistory(0, 0)).isEqualTo(null);
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromCoordsHistory(Mockito.anyDouble(), Mockito.anyDouble());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromCoordsHistory(Mockito.anyDouble(), Mockito.anyDouble());
        checkServiceRequestNums(0, 0, 0, 1);
    }
    ...
    @Test
    void getDataFromLocationForecastAllNull() {
        Mockito.when(owmService
                .getDataFromLocationForecast(Mockito.anyString(), Mockito.anyString()))
                .thenReturn(null);
        Mockito.when(wbService
        ...
        assertThat(cacheService.getDataFromLocationForecast("", "")).isEqualTo(null);
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromLocationForecast(Mockito.anyString(), Mockito.anyString());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromLocationForecast(Mockito.anyString(), Mockito.anyString());
        checkServiceRequestNums(0, 0, 0, 1);
    }
    ...
    @Test
    void getDataFromCoordsForecastAllNull() {
        Mockito.when(owmService
                .getDataFromCoordsForecast(Mockito.anyDouble(), Mockito.anyDouble()))
                .thenReturn(null);
        Mockito.when(wbService
        ...
        assertThat(cacheService.getDataFromCoordsForecast(0, 0)).isEqualTo(null);
        Mockito.verify(owmService, VerificationModeFactory.times(1)).getDataFromCoordsForecast(Mockito.anyDouble(), Mockito.anyDouble());
        Mockito.verify(wbService, VerificationModeFactory.times(1)).getDataFromCoordsForecast(Mockito.anyDouble(), Mockito.anyDouble());
        checkServiceRequestNums(0, 0, 0, 1);
    }
    ...
    private void checkServiceRequestNums(int owm, int wb, int c, int f) {
        assertThat(cacheService.nOWMRequests).isEqualTo(owm);
        assertThat(cacheService.nWBRequests).isEqualTo(wb);
        assertThat(cacheService.nCacheRequests).isEqualTo(c);
        assertThat(cacheService.nFailedRequests).isEqualTo(f);
    }
